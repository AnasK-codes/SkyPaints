...
const PROJECT_TYPES = ['Interior Walls', 'Exterior Walls', 'Wood Finishes', 'Metal Surfaces'];

const PRODUCTS = [
  { id: 'premium', name: 'Premium Luxury Emulsion', price: 450, coverage: 120 },
  { id: 'washable', name: 'Advanced Washable Plastic', price: 300, coverage: 100 },
  { id: 'distemper', name: 'Classic Distemper', price: 150, coverage: 80 },
  { id: 'exterior', name: 'All-Weather Exterior Shield', price: 500, coverage: 60 },
];
...
function CustomSelect({ label, options, value, onChange, className = '' }: any) {
  ...
  return (
    <div className={`space-y-2 relative ${className}`} ref={ref}>
      <label className="font-label text-sm text-on-surface-variant ml-4 block">{label}</label>
      <div className="relative">
        ...
        {isOpen && (
          <div className="absolute z-50 mt-2 w-full bg-[#484452] text-[#f2f0f4] rounded-2xl py-2 shadow-xl border border-white/5 overflow-hidden backdrop-blur-xl animate-fade-in-up" style={{ animationDuration: '0.2s' }}>
            {options.map((option: any, i: number) => {
              const isSelected = (option.name || option) === (value?.name || value);
              return (
                <button
                  key={i}
                  type="button"
                ...
                <div className="relative z-10">
                  ...
                  <div className="space-y-6">
                    <div className="flex justify-between items-end border-b border-white/10 pb-4">
                      <div>
                        <p className="font-label text-white/60 text-sm mb-1">Required Quantity</p>
                        <p className="font-headline text-2xl">~ {estimate ? estimate.quantity : '0'} Litres</p>
                      </div>
                      <p className="font-body text-white/80">{estimate ? formatCurrency(estimate.paintCost) : '₹ 0'}</p>
                    </div>
                    <div className="flex justify-between items-end border-b border-white/10 pb-4">
                      <div>
                        <p className="font-label text-white/60 text-sm mb-1">Estimated Labour</p>
                        <p className="font-headline text-2xl">{estimate ? estimate.labourType : 'Standard Team'}</p>
                      </div>
                      <p className="font-body text-white/80">{estimate ? formatCurrency(estimate.labourCost) : '₹ 0'}</p>
                    </div>
                  </div>
                </div>
                <div className="mt-12 pt-8 border-t border-white/10 relative z-10">
                  <p className="font-label text-white/60 text-sm mb-2">Total Approximate Cost</p>
                  <p className="font-headline text-5xl tracking-tight text-[#f5e0c0]">{estimate ? formatCurrency(estimate.total) : '₹ 0'}</p>
