...
const VASTU_DATA = [
  {
    ...
    badgeBg: "bg-blue-50",
    icon: "water_drop",
    description: "Ideal for living rooms and entrances. Represents career and wealth.",
  ...
  },
  {
    ...
    description: "The sacred zone (Ishan). Best for prayer rooms and meditation spaces.",
  ...
  },
  {
    ...
    badgeBg: "bg-amber-50",
    icon: "air",
    description: "Perfect for family lounges and study areas. Symbolizes health and growth.",
  ...
  },
  {
    ...
    description: "The Agni zone. Perfect for kitchens. Represents passion and vitality.",
  ...
  },
  {
    ...
    badgeBg: "bg-red-50",
    icon: "local_fire_department",
    description: "Best suited for bedrooms and relaxation areas. Represents fame and energy.",
  ...
  },
  {
    ...
    icon: "terrain",
    description: "Grounding zone, ideal for master bedrooms. Represents stability and relationships.",
  ...
  },
  {
    ...
    icon: "star",
    description: "Ideal for dining rooms and children's rooms. Symbolizes creativity and gains.",
  ...
  },
  {
    ...
    badgeBg: "bg-sky-50",
    icon: "air",
    description: "Ideal for living rooms and guest rooms. Represents balance and communication.",
      ...
      <div className="bg-surface-container-lowest rounded-3xl p-8 lg:p-16 border border-outline-variant/10 shadow-ambient">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {VASTU_DATA.map((item, idx) => (
            ...
            >
              <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl ${item.decoColor} to-transparent opacity-30 rounded-bl-full -z-10 transition-transform duration-500 group-hover:scale-[1.5]`}></div>
              
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-2">
                  <span className={`material-symbols-outlined text-[20px] ${item.elementColor}`}>{item.icon}</span>
                  <h3 className="font-headline text-xl lg:text-2xl text-primary">{item.direction}</h3>
                </div>
                <span className={`text-[9px] font-bold ${item.elementColor} ${item.badgeBg} px-2.5 py-1 rounded-full tracking-wider uppercase shadow-sm border border-black/5`}>
                  {item.element}
                </span>
              </div>
                ...
                <div className="flex -space-x-2 mr-3">
                  {item.swatches.map((color, i) => (
                    <div 
                      ...
                      style={{ backgroundColor: color, zIndex: item.swatches.length - i }}
                    ></div>
                  ))}
                </div>
        ...
        <p className="text-center text-xs font-body text-on-surface-variant/50 mt-12 max-w-xl mx-auto">
          Note: Screen colors are indicative. Actual paint shades may vary based on your home's lighting and wall textures.
        </p>
